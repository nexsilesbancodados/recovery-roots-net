...
const rowLabels = [
  { after: 0, text: "Conforto" },
  { after: 1, text: "Natureza" },
  { after: 2, text: "Bem-Estar" },
];
...
const HorizontalLabel = ({ text }: { text: string }) => (
  <div className="my-4 flex items-center gap-4">
    <div className="flex-1 h-px bg-border" />
    <span className="font-display text-xl md:text-2xl font-black text-secondary/60 uppercase tracking-[0.3em]">
      {text}
    </span>
    <div className="flex-1 h-px bg-border" />
  </div>
...
const EstruturaSection = () => {
  ...
  return (
    <section className="relative z-30 py-16 sm:py-24 bg-[hsl(215,10%,98%)] overflow-hidden" id="estrutura">
      {/* Header - centered */}
      <div className="container mx-auto px-4 mb-6 sm:mb-8 text-center">
        <span className="text-primary font-semibold text-xs sm:text-sm uppercase tracking-wider">
          Infraestrutura
        </span>
        <h2 className="text-2xl sm:text-3xl md:text-4xl lg:text-5xl font-display font-black text-secondary mt-2">
          Nossa Estrutura
        </h2>
        <p className="text-muted-foreground mt-3 sm:mt-4 text-sm sm:text-base md:text-lg max-w-xl mx-auto">
      ...
      <div className="hidden sm:block container mx-auto px-4">
        {imageRows.map((row, rowIndex) => (
          <div key={rowIndex}>
            ...
            {/* Horizontal label between rows */}
            {rowLabels[rowIndex] && (
              <HorizontalLabel text={rowLabels[rowIndex].text} />
            )}

            {/* Banners after specific rows */}
            {rowIndex === 0 && (
      ...
      <div className="sm:hidden px-4 space-y-3">
        ...
        <HorizontalLabel text="Bem-Estar" />
