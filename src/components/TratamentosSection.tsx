        ...
        <div className="container mx-auto px-4">
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
            {tratamentos.slice(0, 4).map((tratamento) => (
              <TratamentoCard key={tratamento.id} tratamento={tratamento} onClick={() => setSelectedTratamento(tratamento)} />
            ))}
          </div>
        </div>

        {/* Scrolling ticker strip */}
        <div className="my-4 sm:my-6 overflow-hidden bg-primary/10 py-2">
          <div className="animate-scroll-x flex whitespace-nowrap gap-8 items-center">
            {[...Array(3)].map((_, repeat) => (
              <span key={repeat} className="flex gap-8 items-center">
                {tratamentos.map((t) => (
                  <span key={`${repeat}-${t.id}`} className="text-primary/70 text-xs sm:text-sm font-semibold uppercase tracking-wider flex items-center gap-2">
